...
import { useGetStats, useListVaults } from "@/lib/api-client";
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Link href="/security">
              <span className={cn("bg-card border rounded-xl p-4 cursor-pointer hover:border-primary/30 transition-colors block", weakCount > 0 ? "border-amber-500/30" : "border-border")}>
                ...
                <div className={cn("text-2xl font-bold", weakCount > 0 ? "text-amber-400" : "text-foreground")}>{weakCount}</div>
                <div className="text-xs text-muted-foreground mt-1">{weakCount > 0 ? "Need attention" : "All secure"}</div>
              </span>
            </Link>

            <Link href="/security">
              <span className={cn("bg-card border rounded-xl p-4 cursor-pointer hover:border-primary/30 transition-colors block", reusedCount > 0 ? "border-red-500/30" : "border-border")}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-medium text-muted-foreground">Reused Passwords</span>
                  <AlertTriangle className={cn("w-4 h-4", reusedCount > 0 ? "text-red-400" : "text-muted-foreground")} />
                </div>
                <div className={cn("text-2xl font-bold", reusedCount > 0 ? "text-red-400" : "text-foreground")}>{reusedCount}</div>
                <div className="text-xs text-muted-foreground mt-1">{reusedCount > 0 ? "High risk" : "No duplicates"}</div>
              </span>
            </Link>

            <Link href="/security">
              <span className={cn("bg-card border rounded-xl p-4 cursor-pointer hover:border-primary/30 transition-colors block", strongCount > 0 ? "border-emerald-500/20" : "border-border")}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-medium text-muted-foreground">Strong Passwords</span>
                  <CheckCircle className={cn("w-4 h-4", strongCount > 0 ? "text-emerald-400" : "text-muted-foreground")} />
                </div>
                <div className={cn("text-2xl font-bold", strongCount > 0 ? "text-emerald-400" : "text-foreground")}>{strongCount}</div>
                <div className="text-xs text-muted-foreground mt-1">Out of {loginCount} logins</div>
              </span>
            </Link>
          </div>
          ...
          {vaults.length > 0 && (
            <div>
              <h2 className="text-sm font-medium text-foreground mb-3">Your Vaults</h2>
              <div className="grid grid-cols-3 gap-3">
                {vaults.map((vault) => (
                  <Link key={vault.id} href={`/vault/${vault.id}`}>
                    <span className="bg-card border border-border rounded-xl p-4 hover:border-primary/30 transition-colors cursor-pointer block">
                      ...
                      {vault.description && (
                        <p className="text-xs text-muted-foreground truncate mb-2">{vault.description}</p>
                      )}
                      <div className="text-xs text-muted-foreground">{vault.itemCount} items</div>
                    </span>
                  </Link>
                ))}
              </div>
